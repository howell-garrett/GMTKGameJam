...
using System.Collections;
    ...
    public class ETFXProjectileScript : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            if (hit.collider.CompareTag("Enemy") && isPlayerShot)
            {
                hit.collider.GetComponent<EnemyAttributes>().TakeDamage(damage);
            }
            if (hit.collider.CompareTag("Player") && !isPlayerShot)
            {
                hit.collider.GetComponent<PlayerAttributes>().TakeDamage(damage);
            }
