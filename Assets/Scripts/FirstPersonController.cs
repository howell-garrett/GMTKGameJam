...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (enableJetpack && Input.GetKey(jumpKey) && !isGrounded && jetpackTimer <= 0)
        {
        ...
        #region Shoot
        if (Input.GetMouseButtonDown(0) && rateOfFireReset <= 0) {
            Shoot();
            rateOfFireReset = rateOfFire;
        }
    ...
    private void Jetpack()
    {
        if (!isGrounded)
        {
            rb.AddForce(0f, jetpackPower, 0f, ForceMode.Force);
        }
    }
    ...
    private void Dash()
    {
        ...
        { //only W D
            dashDirection = transform.forward + transform.right;
        }
        else if (Input.GetKey(KeyCode.W) && Input.GetKey(KeyCode.A) && !Input.GetKey(KeyCode.S) && !Input.GetKey(KeyCode.D))
        { //only W A
            dashDirection = transform.forward - transform.right;
    ...
    private void Shoot()
    {
        ...
        shot.GetComponent<ETFXProjectileScript>().isPlayerShot = true;
